...
with Timer():
    for hemi in range(len(src)):
        for vidx in source_to_vert[hemi]:
            ...
            for order, neighbor_scale in zip([1, 2], (0, 0)):
...
# Save the results
with open('results/Experiment_1-10-2_Osc_results.pickle', 'wb') as openfile:
    pickle.dump((all_x_t_n_Osc, all_P_t_n_Osc, em_iters_Osc), openfile)

with open('results/Experiment_1-10-2_Arn_results.pickle', 'wb') as openfile:
